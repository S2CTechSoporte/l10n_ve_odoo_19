...
class AccountMove(models.Model):
    ...
    date_document = fields.Date(string='Document Date', #v17 states={'draft': [('readonly', False)]},
    ...
    nro_ctrl = fields.Char(
        ...
        size=32,
        help=(
            "Número utilizado para gestionar facturas preimpresas, por ley Necesito poner aquí este"
    ...
    identification_id1 = fields.Char(string='Documento de Identidad', related='partner_id.identification_id',
                                     store=True, #v17 states={'draft': [('readonly', True)]}
                                     )
    ...
    vat_apply = fields.Boolean(
        ...
        # v17 states={'draft': [('readonly', False)]},
        ...
        if not vals.get('check_fiscal'):
            if vals.get('invoice_date') and isinstance(vals.get('invoice_date'), str):
                ...
                if vals.get('invoice_date'):
                    ...
                    fecha_factura = False
            if vals.get('date') and isinstance(vals.get('date'), str):
    ...
    def ret_and_reconcile(self, pay_amount, pay_account_id,
        ...
        if module_dual_currency:
            l1['amount_residual_fcurrency'] = direction * (pay_amount / invoice.tax_today)
        lines = [(0, 0, l1)]
        ...
        if module_dual_currency:
            l2[0][2]['amount_residual_fcurrency'] = direction * ((deb if deb > 0 else cred) / invoice.tax_today)
        lines += l2
        ...
        if module_dual_currency:
            payment_lines.amount_residual_fcurrency = payment_lines.amount_residual / invoice.tax_today
            print('monto residual al crear el pago', payment_lines.amount_residual_fcurrency)
        ...
        if module_dual_currency:
            if type(results) is dict and 'partials' in results:
                if results['partials'].amount_fcurrency == 0:
                    monto_fcurrency = abs(payment_lines.amount_residual_fcurrency)
                    results['partials'].write({'amount_fcurrency': monto_fcurrency})
                    payment_lines._compute_amount_residual_fcurrency()
