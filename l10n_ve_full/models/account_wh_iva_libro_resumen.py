...
class LibroVentasModelo(models.Model):
    _name = "account.wh.iva.libro.pdf.resumen"

    _description = "Líneas PDF resumen libro retención IVA"
...
class resumen_libros(models.TransientModel):
    _name = "account.wh.iva.libro.resumen"  ## = nombre de la carpeta.nombre del archivo deparado con puntos

    _description = "Asistente resumen libro retención IVA"
    ...
    report = fields.Binary('Descargar', readonly=True)
    name = fields.Char('File Name', size=32)
    company_id = fields.Many2one('res.company', 'Compañía', default=lambda self: self.env.company)

    line = fields.Many2many(
        comodel_name='account.wh.iva.libro.pdf.resumen',
        relation='acc_wh_iva_libro_resu_pdf_rel',
        column1='wizard_id',
        column2='pdf_id',
        string='Lineas',
    )
    ...
    def ret_iva(self):  # calcula las retenciones que han hecho los clientes
        ...
        for det in cursor_resumen:
            if det.vat_ret_id.state == "confirmed":
    ...
    def get_invoice(self, accion):
        t = self.env['account.wh.iva.libro.pdf.resumen']
        d = t.search([])
        # d.unlink()
        if accion == "factura":
            cursor_resumen = self.env['account.move.line.resumen'].search([
    ...
    def generate_xls_report(self):
        self.env['account.wh.iva.libro.pdf.resumen'].search([]).unlink()
        # self.get_invoice()
        ...
        header_style_c = xlwt.easyxf(
            "font: name Helvetica size 10 px, height 170; borders: left thin, right thin, top thin, bottom thin; align: horiz center")
        header_style_r = xlwt.easyxf(
            "font: name Helvetica size 10 px, height 170; borders: left thin, right thin, top thin, bottom thin; align: horiz right")
        ...
        sub_total11 = diccionario['alicuota_general'] + diccionario['alicuota_adicional'] + diccionario[
            'alicuota_reducida']
        row = row + 1
        ws1.write_merge(row, row, 4, 4, (row - 5), header_style_c)
        ...
        ws1.write_merge(row, row, 4, 4, (row - 6), header_style_c)
        ws1.write_merge(row, row, 5, 7,
                        "Certificados de Débitos Fiscales Exonerados (emitidos de entes exonerados) Registrados en el periodo",
                        header_style)
        ws1.col(5).width = int(
            len('Certificados de Débitos Fiscales Exonerados (emitidos de entes exonerados) Registrados en el periodo') * 128)
        ws1.write_merge(row, row, 8, 8, "0,00", header_style_r)
        ws1.write_merge(row, row, 9, 9, "0,00", header_style_r)
        row = row + 1
        total22 = sub_total22
        ...
        return {
            ...
            'res_model': 'account.wh.iva.libro.resumen',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
