...
class AccountWhIslrDoc(models.Model):
    ...
    concept_ids = fields.One2many(
        ...
        readonly=True, #v17 states={'draft': [('readonly', False)]},
    ...
    @api.constrains('partner_id', 'type', 'company_id')
    def _constrains_partner_is_income_wh_agent(self):
        for rec in self:
            if not rec.partner_id or not rec.company_id or not rec.type:
                continue
            if not rec._check_partner():
                raise ValidationError('Error! El socio debe ser un agente de retención de ingresos.')
