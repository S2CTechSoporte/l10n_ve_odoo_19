...
class AccountPaymentRegister(models.TransientModel):
    ...
    fcurrency_id = fields.Many2one("res.currency",string="Divisa de Referencia")
    currency_id_name = fields.Char(related="currency_id.name")
    amount_residual_fcurrency = fields.Monetary(currency_field='fcurrency_id', string='Adeudado Divisa Ref.', readonly=True)
    payment_difference_bs = fields.Monetary(string="Diferencia Bs", currency_field='company_currency_id')
    payment_difference_fcurrency = fields.Monetary(string="Diferencia $", currency_field='fcurrency_id')
    journal_id_dif = fields.Many2one('account.journal', 'Diario de diferencia', store=True,
                                 domain="[('company_id', '=', company_id)]")
    amount_fcurrency = fields.Monetary(currency_field='fcurrency_id', string='Importe $', readonly=True)
    ...
    mount_igtf = fields.Monetary(currency_field='currency_id', string='Importe IGTF', readonly=False)
        ...
        for wizard in self:
            if wizard.source_currency_id == wizard.currency_id:
                ...
                if wizard.tax_today == wizard.tax_invoice:
                    ...
                    if wizard.source_currency_id == wizard.company_id.fcurrency_id:
                        wizard.amount = wizard.amount_residual_fcurrency
                    else:
                        wizard.amount = wizard.amount_residual_fcurrency * wizard.tax_today
    ...
    @api.depends('can_edit_wizard', 'amount','tax_today')
    def _compute_payment_difference(self):
        for wizard in self:
            wizard.payment_difference = 0 # valor inicial
            wizard.amount_fcurrency = wizard.amount / (wizard.tax_today if wizard.tax_today > 0 else 1)
            if wizard.source_currency_id == wizard.currency_id:
                # Same currency.
                wizard.payment_difference = wizard.source_amount_currency - wizard.amount
                wizard.payment_difference_fcurrency = wizard.amount_residual_fcurrency - (wizard.amount / (wizard.tax_today if wizard.tax_today > 0 else 1))
                wizard.payment_difference_bs = 0
                if wizard.currency_id == wizard.company_id.fcurrency_id:
                    wizard.payment_difference_fcurrency = wizard.amount_residual_fcurrency - wizard.amount
                    wizard.payment_difference_bs = (wizard.amount_residual_fcurrency / wizard.tax_invoice) - (wizard.amount / (wizard.tax_today if wizard.tax_today > 0 else 1))
                print('diferencia 1')
            elif wizard.currency_id == wizard.company_id.currency_id:
                # Payment expressed on the company's currency.
                if wizard.source_currency_id == wizard.company_id.currency_id:
                ...
                if wizard.tax_today == wizard.tax_invoice and wizard.amount_residual_fcurrency == wizard.amount and wizard.currency_id == wizard.company_id.fcurrency_id:
                    wizard.payment_difference_bs = 0
                else:
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        if not self.tax_today:
            if lines[0].company_id.fcurrency_id != self.env.ref('base.USD'):
                tax_today = lines[0].company_id.fcurrency_id.rate
            else:
                tax_today = lines[0].company_id.fcurrency_id.inverse_rate
        else:
            tax_today = self.tax_today

        fcurrency_id = lines[0].fcurrency_id
        amount_residual_fcurrency = lines[0].move_id.amount_residual_fcurrency
        ...
        return {
            ...
            'fcurrency_id': fcurrency_id.id,
            'amount_residual_fcurrency': amount_residual_fcurrency,
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        payment_vals = {
            ...
            'tax_today': self.tax_today,
            'fcurrency_id': self.fcurrency_id.id,
            'aplicar_igtf_divisa': self.aplicar_igtf_divisa,
    ...
    def _xxxxxxxxxxxxxxxxxxxxxxx_create_payments(self):
        ...
        for partial in partials:
            if partial.amount_fcurrency == 0:
                monto_fcurrency = 0
                to_reconcile = payments.move_id.line_ids.filtered_domain([('account_id', '=', self.line_ids[0].account_id.id)])

                if abs(self.line_ids[0].amount_residual_fcurrency) > 0:
                    #print("1")
                    if abs(self.line_ids[0].amount_residual_fcurrency) > to_reconcile.amount_residual_fcurrency:
                        #print("2")
                        monto_fcurrency = abs(to_reconcile.amount_residual_fcurrency)
                    else:
                        #print("3")
                        monto_fcurrency = abs(self.line_ids[0].amount_residual_fcurrency)
                partial.write({'amount_fcurrency': monto_fcurrency})

                to_reconcile._compute_amount_residual_fcurrency()
                print('escribe el parcial: %s' % monto_fcurrency)
        ...
        if self.source_amount == 0:
            ...
            tmp_d = l_cliente.debit_fcurrency
            tmp_c = l_cliente.credit_fcurrency
            l_cliente.with_context(check_move_validity=False).debit = 0
            l_cliente.with_context(check_move_validity=False).credit = 0
            l_cliente.with_context(check_move_validity=False).debit_fcurrency = tmp_d
            l_cliente.with_context(check_move_validity=False).credit_fcurrency = tmp_c
            ...
            self.env['account.partial.reconcile'].create([{
                'amount': 0,
                'amount_fcurrency': self.amount_residual_fcurrency if (tmp_d > self.amount_residual_fcurrency or tmp_c > self.amount_residual_fcurrency) else (tmp_d if tmp_d > 0 else tmp_c),
            ...
            print("self.payment_difference_fcurrency", self.payment_difference_fcurrency)
            ...
            if not self.currency_id.is_zero(self.payment_difference) and self.payment_difference_bs < 0 and self.payment_difference_handling == 'reconcile':
                ...
                move = {'ref': self.writeoff_label + ' de ' + self.communication,
                        ...
                        'type_name': 'entry',
                        'tax_today': 0,
                        'fcurrency_id': self.currency_id.id,
                        }
                ...
                #(payment_lines + to_reconcile).reconcile()
            elif not self.currency_id.is_zero(self.payment_difference) and self.payment_difference_bs == 0 and self.payment_difference_fcurrency == 0\
            ...
            elif self.payment_difference > 0 and self.payment_difference_bs > 0 and self.payment_difference_fcurrency == 0 and self.payment_difference_handling == 'reconcile' and self.currency_id == self.company_id.currency_id:
                move = {'ref': self.writeoff_label + ' de ' + self.communication,
                        'line_ids': [(0, 0, {
                            'amount_currency': self.payment_difference,
                            'debit': self.payment_difference_bs,
                            'credit': 0,
                            'debit_fcurrency': self.payment_difference,
                            'credit_fcurrency': 0,
                            'account_id': self.writeoff_account_id.id if payments.payment_type == 'inbound' else
                            self.line_ids[0].account_id.id,
                            'partner_id': False if payments.payment_type == 'inbound' else self.partner_id.id,
                            ...
                            'debit': 0,
                            'debit_fcurrency': 0,
                            'credit_fcurrency': self.payment_difference,
            ...
            elif self.payment_difference > 0 and self.payment_difference_fcurrency > 0 and self.payment_difference_handling == 'reconcile' and self.currency_id == self.source_currency_id:
                move = {'ref': self.writeoff_label + ' de ' + self.communication,
                        'line_ids': [(0, 0, {
                            'debit': self.payment_difference,
                            'credit': 0,
                            'debit_fcurrency': self.payment_difference_fcurrency,
                            'credit_fcurrency': 0,
                            'account_id': self.writeoff_account_id.id if payments.payment_type == 'inbound' else
                            self.line_ids[0].account_id.id,
                            'partner_id': False if payments.payment_type == 'inbound' else self.partner_id.id,
                            ...
                            'debit': 0,
                            'debit_fcurrency': 0,
                            'credit_fcurrency': self.payment_difference_fcurrency,
        ...
        if 'line_ids' in fields_list and 'line_ids' not in res:
            ...
            for line in lines:
                ...
                if line.account_type not in ('asset_receivable', 'liability_payable'):
                    continue
                if line.currency_id:
                    if line.move_id.amount_residual_fcurrency == 0.0:
                        continue
                else:
                    if line.company_currency_id.is_zero(line.amount_residual) and line.move_id.amount_residual_fcurrency == 0.0:
                        continue
    ...
    #         #payment_vals['fcurrency_id'] = self.fcurrency_id.id
    ...
    #     payment_vals_list[0]['fcurrency_id'] = self.fcurrency_id.id
