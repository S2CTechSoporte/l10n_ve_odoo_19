...
class StockLandedCost(models.Model):
    ...
    currency_id = fields.Many2one(
        ...
        required=True,
        related=None, # permite cambiar la moneda
    ...
    def _get_default_tasa(self):
        for rec in self:
            return self.env.company.fcurrency_id.inverse_rate
    ...
    #         cost_to_add_byproduct_fcurrency = defaultdict(lambda: 0.0)
    #         asientos = []
    ...
    #             cost_to_add_fcurrency = (remaining_qty / line.move_id.product_qty) * line.additional_landed_cost_fcurrency
    #             if not cost.company_id.currency_id.is_zero(cost_to_add):
    #                 layer = {
    #                     'value': cost_to_add,
    #                     'value_fcurrency': cost_to_add_fcurrency,
    #                     'unit_cost': cost_to_add / line.quantity,
    #                     'unit_cost_fcurrency': cost_to_add_fcurrency / line.quantity,
    #                     'quantity': 0,
    #                     'remaining_qty': 0,
    ...
    #                 linked_layer.remaining_value_fcurrency += cost_to_add_fcurrency
    #                 valuation_layer_ids.append(valuation_layer.id)
    #             # Update the AVCO
    #             product = line.move_id.product_id
    #             if product.cost_method == 'average':
    #                 cost_to_add_byproduct[product] += cost_to_add
    #                 cost_to_add_byproduct_fcurrency[product] += cost_to_add_fcurrency
    ...
    #                 'fcurrency_id': self.env.company.fcurrency_id.id,
    #                 'move_type': 'entry',
    #             }
    ...
    #                 product.with_company(cost.company_id).sudo().with_context(disable_auto_svl=True).standard_price_fcurrency += \
    #                     cost_to_add_byproduct_fcurrency[product] / product.quantity_svl
    ...
    #             'former_cost_fcurrency': sum(move.stock_valuation_layer_ids.mapped('value_fcurrency')),
    ...
    #                         towrite_dict_fcurrency[valuation.id] = value / line.tax_today
    #                     else:
    #                         towrite_dict[valuation.id] += value
    #                         towrite_dict_fcurrency[valuation.id] += value / line.tax_today
    #     for key, value in towrite_dict.items():
    #         AdjustementLines.browse(key).write({'additional_landed_cost': value, 'additional_landed_cost_fcurrency': towrite_dict_fcurrency[key]})
    #     return True
