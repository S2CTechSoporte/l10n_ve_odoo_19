...
class AccountMove(models.Model):
    ...
    amount_ref = fields.Monetary(string="Importe referencia", currency_field='fcurrency_id' )
    ...
    def _get_default_tasa(self):
        if self.env.company.fcurrency_id != self.env.ref('base.USD'):
            return self.env.company.fcurrency_id.rate
        else:
            return self.env.company.fcurrency_id.inverse_rate

    @api.depends('fcurrency_id','currency_id','amount','tax_today')
    def _currency_equal(self):
        for rec in self:
            currency_equal = rec.currency_id_company != rec.currency_id
            if currency_equal:
                # rec.amount_local = rec.amount * rec.tax_today
                rec.amount_local = rec.currency_id._convert(rec.amount,  rec.currency_id_company, rec.company_id,  rec.date, True)
                ...
                rec.amount_ref = rec.currency_id._convert(rec.amount, rec.fcurrency_id, rec.company_id,  rec.date, True)
