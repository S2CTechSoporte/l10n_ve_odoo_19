...
class PosPayment(models.Model):
    ...
    def name_get(self):
        ...
        for payment in self:
            if payment.name:
                res.append((payment.id, '%s %s - %s' % (payment.name, formatLang(self.env, payment.amount, currency_obj=payment.currency_id), formatLang(self.env, payment.amount_ref, currency_obj=payment.fcurrency_id))))
            else:
                res.append((payment.id, '%s - %s' % (formatLang(self.env, payment.amount, currency_obj=payment.currency_id), formatLang(self.env, payment.amount_ref, currency_obj=payment.fcurrency_id))))
        return res